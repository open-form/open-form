---
title: Registries
description: Consuming and creating OpenForm artifact registries
ogTitle: OpenForm Registries
...
Use [`ofm search`](/cli/commands/search) to query a registry:
...
Use [`ofm add`](/cli/commands/add) to install an artifact. The CLI resolves the namespace to a registry, downloads the definition, writes it to your artifacts directory, and updates the lock file:
...
1. **Create the registry** — [`ofm registry make`](/cli/commands/registry/make) generates a `registry.json` with your namespace and metadata.

2. **Add artifacts to the catalog** — [`ofm registry catalog`](/cli/commands/registry/catalog) manages the artifacts listed in the index.

3. **Compile** — [`ofm registry compile`](/cli/commands/registry/compile) validates all artifacts and prepares the registry for publishing.
