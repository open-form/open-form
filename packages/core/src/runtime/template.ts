...
import type { Layer, InlineLayer, FileLayer, Form, Resolver } from '@open-form/types'
import type { FormTemplate } from './renderer'
...
/**
 * Resolve a Layer into a runtime FormTemplate.
 *
 * This function:
 *  - handles inline layers directly
 *  - loads file-based layers via the Resolver
 */
export async function resolveLayerToFormTemplate(
  layer: Layer,
  resolver: Resolver
 ...
 * @returns The resolved FormTemplate
 */
export async function resolveFormLayer(
  ...
  if (!layer) {
    throw new Error(
      `Layer "${key}" not found in form. Available layers: ${Object.keys(form.layers).join(', ')}`
    )
  }

  return resolveLayerToFormTemplate(layer, resolver)
}

/**
 * @deprecated Use resolveFormLayer instead.
 */
export const resolveFormTemplate = resolveFormLayer
