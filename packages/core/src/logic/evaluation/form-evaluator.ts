 ...
 *
 * Produces a FormRuntimeState with the evaluated visible/required/disabled
 * states for all fields and annexes.
 */

import type { Form, Field, FieldsetField, Annex } from '@open-form/types'
import type {
  FormRuntimeState,
  FieldRuntimeState,
  AnnexRuntimeState,
 ...
 * - disabled defaults to false (allow editing)
 */
const DEFAULTS = {
  visible: true,
  required: false,
...
function evaluateFields(
  ...
  for (const [fieldId, field] of Object.entries(fields)) {
    ...
    const disabled = evaluateBooleanExpression(field.disabled, context, DEFAULTS.disabled)
 ...
 *
 * @param annexes - Annex definitions
 * @param context - Evaluation context
 * @param state - Evaluation state to accumulate results
 */
function evaluateAnnexes(
  ...
  for (const annex of annexes) {
    ...
    state.annexes.set(annex.id, annexState)
  }
}
