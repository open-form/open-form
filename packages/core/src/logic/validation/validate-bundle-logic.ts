...
import type { Bundle, BundleContentItem, CondExpr } from '@open-form/types'
...
import { buildBundleTypeEnvironment, validateBooleanType, topologicalSortLogicKeys } from '../types'
 ...
 *   title: 'Test',
 *   logic: {
 *     needsForm: 'forms.main.fields.required.value == true'
 *   },
 *   contents: [
...
export function validateBundleLogic(
  ...
  if (bundle.logic) {
    for (const [key, expr] of Object.entries(bundle.logic)) {
      if (!validateExpression(expr, ['logic', key], validVariables, issues, collectAllErrors)) {
        if (!collectAllErrors) {
          return { issues }
        }
      }
