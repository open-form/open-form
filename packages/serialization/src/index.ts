/**
 * @open-form/serialization
 *
 * Primitive serialization for OpenForm framework
 * Locale and region-aware conversion of primitives to strings
 */

import type { Money, Address, Phone, Person, Organization } from '@open-form/types'
import type { Party } from '@open-form/types'
import type { FormatterRegistry, FormatterConfig } from '@open-form/types'

// Re-export types from @open-form/types for convenience
export type { FormatterRegistry, FormatterConfig } from '@open-form/types'

// ============================================================================
// Default USA Formatters (USA-locale implementation)
// ============================================================================

/**
 * Default formatter registry using USA locale and regional formatting.
 * This is the reference implementation.
 */
export const usaFormatters: FormatterRegistry = {
	formatMoney(value: Money | number | Partial<Money>): string {
		if (typeof value === 'number') {
			return new Intl.NumberFormat('en-US', {
				style: 'currency',
				currency: 'USD',
			}).format(value)
		}

		if (!value || typeof value !== 'object') {
			return ''
		}

		const amount = (value as Money).amount ?? 0
		const currency = (value as Money).currency || 'USD'

		return new Intl.NumberFormat('en-US', {
			style: 'currency',
			currency,
		}).format(amount)
	},

	formatAddress(value: Address | Partial<Address>): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		const parts: string[] = []

		if (value.line1) parts.push(value.line1)
		if (value.line2) parts.push(value.line2)

		const localityRegionZip: string[] = []
		if (value.locality) localityRegionZip.push(value.locality)
		if (value.region) localityRegionZip.push(value.region)
		if (value.postalCode) localityRegionZip.push(value.postalCode)

		if (localityRegionZip.length > 0) {
			parts.push(localityRegionZip.join(', '))
		}

		if (value.country) parts.push(value.country)

		return parts.join(', ')
	},

	formatPhone(
		value:
			| Phone
			| string
			| Partial<Phone>
			| { number?: string; countryCode?: string; extension?: string }
	): string {
		if (typeof value === 'string') {
			// Simple formatting for string phone numbers
			const cleaned = value.replace(/\D/g, '')
			if (cleaned.length === 10) {
				return `(${cleaned.slice(0, 3)}) ${cleaned.slice(3, 6)}-${cleaned.slice(6)}`
			}
			if (cleaned.length === 11) {
				return `+${cleaned[0]} (${cleaned.slice(1, 4)}) ${cleaned.slice(4, 7)}-${cleaned.slice(7)}`
			}
			return value
		}

		if (!value || typeof value !== 'object') {
			return ''
		}

		const parts: string[] = []

		// Handle both schema format (number with + prefix) and legacy format (countryCode + number)
		const legacyPhone = value as { countryCode?: string }
		if (legacyPhone.countryCode) {
			parts.push(`+${legacyPhone.countryCode}`)
		}

		const phoneNumber = (value as Phone).number
		if (phoneNumber) {
			// If it's already E.164 format (starts with +), use it directly
			if (phoneNumber.startsWith('+')) {
				parts.push(phoneNumber)
			} else {
				// Otherwise, format it
				const cleaned = phoneNumber.replace(/\D/g, '')
				if (cleaned.length === 10) {
					parts.push(`(${cleaned.slice(0, 3)}) ${cleaned.slice(3, 6)}-${cleaned.slice(6)}`)
				} else {
					parts.push(phoneNumber)
				}
			}
		}

		if ((value as Phone).extension) {
			parts.push(`ext. ${(value as Phone).extension}`)
		}

		return parts.join(' ')
	},

	formatPerson(
		value:
			| Person
			| Partial<Person>
			| {
					fullName?: string
					title?: string
					firstName?: string
					middleName?: string
					lastName?: string
					suffix?: string
			  }
	): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		// If fullName is provided, use it directly (it's required in Person schema)
		if (value.fullName) {
			return value.fullName
		}

		// Otherwise, construct name from optional components
		const nameParts: string[] = []

		if (value.title) nameParts.push(value.title)
		if (value.firstName) nameParts.push(value.firstName)
		if (value.middleName) nameParts.push(value.middleName)
		if (value.lastName) nameParts.push(value.lastName)
		if (value.suffix) nameParts.push(value.suffix)

		return nameParts.join(' ')
	},

	formatOrganization(
		value:
			| Organization
			| Partial<Organization>
			| { name?: string; ein?: string; email?: string; phone?: string }
	): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		const parts: string[] = []

		if ((value as Organization).name) parts.push((value as Organization).name)

		const details: string[] = []
		if ((value as Organization).taxId) details.push(`Tax ID: ${(value as Organization).taxId}`)

		if (details.length > 0) {
			parts.push(`(${details.join(', ')})`)
		}

		return parts.join(' ')
	},

	formatParty(value: Party | Partial<Party>): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		// Party is a union of Person | Organization, so handle both
		if ('firstName' in value || 'lastName' in value) {
			return usaFormatters.formatPerson(value as Person | Partial<Person>)
		}

		if ('name' in value && !('firstName' in value)) {
			return usaFormatters.formatOrganization(value as Organization | Partial<Organization>)
		}

		return ''
	},
}

// ============================================================================
// EU Formatters (Europe-locale implementation)
// ============================================================================

/**
 * Formatter registry using EU locale and regional formatting.
 * Uses EUR currency, EU address format, and E.164 phone format.
 */
export const euFormatters: FormatterRegistry = {
	formatMoney(value: Money | number | Partial<Money>): string {
		if (typeof value === 'number') {
			return new Intl.NumberFormat('de-DE', {
				style: 'currency',
				currency: 'EUR',
			}).format(value)
		}

		if (!value || typeof value !== 'object') {
			return ''
		}

		const amount = (value as Money).amount ?? 0
		const currency = (value as Money).currency || 'EUR'

		return new Intl.NumberFormat('de-DE', {
			style: 'currency',
			currency,
		}).format(amount)
	},

	formatAddress(value: Address | Partial<Address>): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		const parts: string[] = []

		if (value.line1) parts.push(value.line1)
		if (value.line2) parts.push(value.line2)

		// EU format: postal code, city
		const postalCity: string[] = []
		if (value.postalCode) postalCity.push(value.postalCode)
		if (value.locality) postalCity.push(value.locality)
		if (value.region) postalCity.push(value.region)

		if (postalCity.length > 0) {
			parts.push(postalCity.join(' '))
		}

		if (value.country) parts.push(value.country)

		return parts.join(', ')
	},

	formatPhone(
		value:
			| Phone
			| string
			| Partial<Phone>
			| { number?: string; countryCode?: string; extension?: string }
	): string {
		if (typeof value === 'string') {
			// Return as-is, assume it's already in E.164 format or properly formatted
			return value
		}

		if (!value || typeof value !== 'object') {
			return ''
		}

		const parts: string[] = []

		// Handle both schema format (number with + prefix) and legacy format (countryCode + number)
		const phoneNumber = (value as Phone).number
		if (phoneNumber) {
			// If it's already E.164 format (starts with +), use it directly
			if (phoneNumber.startsWith('+')) {
				parts.push(phoneNumber)
			} else {
				// Otherwise, assume it's in E.164 already or return as-is
				parts.push(phoneNumber)
			}
		}

		if ((value as Phone).extension) {
			parts.push(`ext. ${(value as Phone).extension}`)
		}

		return parts.join(' ')
	},

	formatPerson(
		value:
			| Person
			| Partial<Person>
			| {
					fullName?: string
					title?: string
					firstName?: string
					middleName?: string
					lastName?: string
					suffix?: string
			  }
	): string {
		// Same as USA implementation
		return usaFormatters.formatPerson(value)
	},

	formatOrganization(
		value:
			| Organization
			| Partial<Organization>
			| { name?: string; ein?: string; email?: string; phone?: string }
	): string {
		// Same as USA implementation
		return usaFormatters.formatOrganization(value)
	},

	formatParty(value: Party | Partial<Party>): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		// Party is a union of Person | Organization, so handle both
		if ('firstName' in value || 'lastName' in value) {
			return euFormatters.formatPerson(value as Person | Partial<Person>)
		}

		if ('name' in value && !('firstName' in value)) {
			return euFormatters.formatOrganization(value as Organization | Partial<Organization>)
		}

		return ''
	},
}

// ============================================================================
// International Formatters (Generic international implementation)
// ============================================================================

/**
 * Formatter registry using generic international standards.
 * Uses E.164 phone format, minimal assumptions about address structure.
 */
export const intlFormatters: FormatterRegistry = {
	formatMoney(value: Money | number | Partial<Money>): string {
		if (typeof value === 'number') {
			// Use generic number formatting without currency assumptions
			return new Intl.NumberFormat('en-US', {
				minimumFractionDigits: 2,
				maximumFractionDigits: 2,
			}).format(value)
		}

		if (!value || typeof value !== 'object') {
			return ''
		}

		const amount = (value as Money).amount ?? 0
		const currency = (value as Money).currency || ''

		const formatted = new Intl.NumberFormat('en-US', {
			minimumFractionDigits: 2,
			maximumFractionDigits: 2,
		}).format(amount)

		return currency ? `${currency} ${formatted}` : formatted
	},

	formatAddress(value: Address | Partial<Address>): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		const parts: string[] = []

		if (value.line1) parts.push(value.line1)
		if (value.line2) parts.push(value.line2)
		if (value.locality) parts.push(value.locality)
		if (value.region) parts.push(value.region)
		if (value.postalCode) parts.push(value.postalCode)
		if (value.country) parts.push(value.country)

		return parts.filter(Boolean).join(', ')
	},

	formatPhone(
		value:
			| Phone
			| string
			| Partial<Phone>
			| { number?: string; countryCode?: string; extension?: string }
	): string {
		if (typeof value === 'string') {
			// Return string as-is
			return value
		}

		if (!value || typeof value !== 'object') {
			return ''
		}

		const parts: string[] = []

		// Prefer E.164 format (number field)
		const phoneNumber = (value as Phone).number
		if (phoneNumber) {
			parts.push(phoneNumber)
		}

		if ((value as Phone).extension) {
			parts.push(`ext. ${(value as Phone).extension}`)
		}

		return parts.join(' ')
	},

	formatPerson(
		value:
			| Person
			| Partial<Person>
			| {
					fullName?: string
					title?: string
					firstName?: string
					middleName?: string
					lastName?: string
					suffix?: string
			  }
	): string {
		// Same as USA implementation
		return usaFormatters.formatPerson(value)
	},

	formatOrganization(
		value:
			| Organization
			| Partial<Organization>
			| { name?: string; ein?: string; email?: string; phone?: string }
	): string {
		// Same as USA implementation
		return usaFormatters.formatOrganization(value)
	},

	formatParty(value: Party | Partial<Party>): string {
		if (!value || typeof value !== 'object') {
			return ''
		}

		// Party is a union of Person | Organization, so handle both
		if ('firstName' in value || 'lastName' in value) {
			return intlFormatters.formatPerson(value as Person | Partial<Person>)
		}

		if ('name' in value && !('firstName' in value)) {
			return intlFormatters.formatOrganization(value as Organization | Partial<Organization>)
		}

		return ''
	},
}

// ============================================================================
// Formatter Factory
// ============================================================================

/**
 * Create a formatter registry with the specified locale and regional format options.
 *
 * @param config - Configuration with locale and regional format preferences
 * @returns A FormatterRegistry implementation
 *
 * @example
 * ```typescript
 * // Create USA formatters
 * const formatters = createFormatters({ regionFormat: 'US' })
 *
 * // Create EU formatters
 * const euFormatters = createFormatters({ regionFormat: 'EU' })
 *
 * // Create international formatters
 * const intlFormatters = createFormatters({ regionFormat: 'intl' })
 * ```
 */
export function createFormatters(config: FormatterConfig = {}): FormatterRegistry {
	const { regionFormat = 'US' } = config

	switch (regionFormat) {
		case 'EU':
			return euFormatters
		case 'intl':
			return intlFormatters
		case 'US':
		default:
			return usaFormatters
	}
}
